...
def auto_configure(report: dict, node: NodeIdentity | None = None):
    ...
    if use_llm:
        ...
        cognition.update_system_prompt(
            f"""你是一台服务器的自主监控 AI 内核。每 5 秒采集一次系统指标。

当前可用的行动适配器：
{actions_desc}

请分析系统状态，按 JSON 格式输出决策。
JSON 格式:
{{"analysis":"分析","decisions":[{{"intent":"act/sleep/escalate","reason":"理由","actions":[{{"type":"行动类型","target":"适配器名","params":{{}}}}],"confidence":0.0-1.0}}]}}"""
        )
        llm = adapter
        ...
        if report["sensors"].get("psutil"):
            ...
            for r in [
                Rule("cpu_warning", cpu_hi(85), 80, [Action("alert", "alert_logger", {"level": "warning", "message": "CPU > 85%"})]),
                Rule("cpu_critical", cpu_hi(95), 95, [Action("alert", "alert_logger", {"level": "critical", "message": "CPU 过载!"})]),
                Rule("mem_warning", mem_hi(85), 85, [Action("alert", "alert_logger", {"level": "warning", "message": "内存 > 85%"})]),
                Rule("mem_critical", mem_hi(93), 95, [Action("alert", "alert_logger", {"level": "critical", "message": "内存即将耗尽!"}), Action("adjust", "system_log", {"command": "cleanup"})]),
                Rule("disk_space", dsk_hi(88), 80, [Action("alert", "alert_logger", {"level": "warning", "message": "磁盘 > 88%"})]),
                Rule("zombie_procs", zombie_gt(0), 70, [Action("alert", "alert_logger", {"level": "info", "message": "僵尸进程"})]),
            ]:
...
def _start_web():
    ...
    log.info("Web 面板启动于 http://0.0.0.0:8764")
    uvicorn.run(app, host="0.0.0.0", port=8764, log_level="warning")


async def main():
    print("=" * 58)
    print("  衍灵内核 — 自动环境适配")
    print("  Web 面板: http://localhost:8764")
    print("=" * 58)
